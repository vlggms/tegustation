			...
			if(SCREWED_CORE)
				...
				if(istype(P, /obj/item/stack/cable_coil))
					...
					if(C.get_amount() >= 5)
						...
						if(do_after(user, (20 / SKILL_CHECK_VALUE(user, "engineering", 4)), target = src) && state == SCREWED_CORE && C.use(5))
			...
			if(CABLED_CORE)
				...
				if(istype(P, /obj/item/stack/sheet/rglass))
					...
					if(G.get_amount() >= 2)
						...
						if(do_after(user, (20 / SKILL_CHECK_VALUE(user, "engineering", 4)), target = src) && state == CABLED_CORE && G.use(2))
