...
/obj/effect/rune
	...
	var/requires_full_power = FALSE //requires full power to draw or invoke  //tegu edit
...
/obj/effect/rune/convert
	...
	req_cultists = 1
	rune_in_use = FALSE
	requires_full_power = TRUE //tegu edit
...
/obj/effect/rune/apocalypse/invoke(list/invokers)
	...
	var/mob/living/user = invokers[1]
	var/datum/antagonist/cult/user_antag = user.mind.has_antag_datum(/datum/antagonist/cult,TRUE)
	if(!user_antag.cult_team)//Tegu edit start
		to_chat(user, "<span class='cultlarge'>You can't seem to make the arcane links to your fellows that you'd need to use this.</span>") //Tegu edit end
	var/datum/objective/eldergod/summon_objective = locate() in user_antag.cult_team.objectives
