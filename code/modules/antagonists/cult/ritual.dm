...
/obj/item/melee/cultblade/dagger/proc/scribe_rune_attempt(mob/living/user)
	...
	if(!rune_to_scribe)
		return
		if(!iscultist(user, initial(rune_to_scribe.requires_full_power))) //tegu edit start
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie))
		var/datum/objective/eldergod/summon_objective = locate() in user_antag.cult_team?.objectives//tegu edit
		var/datum/objective/sacrifice/sac_objective = locate() in user_antag.cult_team?.objectives//Tegu edit
		if(!summon_objective)
